package cn.com.jcs.leetCode.problem860.lemonadeChange;
 ...
 * Example 1:
 * 
 * Input: [5,5,5,10,20] Output: true Explanation: From the first 3 customers, we
 ...
 * 
 * Input: [5,5,10,10,20] Output: false Explanation: From the first two customers
...
public class Solution {
	public boolean lemonadeChange(int[] bills) {
		List<Integer> getBills = new ArrayList<Integer>();
		for (int bill : bills) {
			if (bill == 5) {
				getBills.add(5);
			} else if (bill == 10) {
				if (!getBills.contains(5)) {
					return false;
				} else {
					getBills.remove(new Integer(5));
					getBills.add(10);
				}
			} else if (bill == 20) {
				if (getBills.contains(10)) {
					getBills.remove(new Integer(10));
					if (!getBills.contains(5)) {
						return false;
					} else {
						getBills.remove(new Integer(5));
