package cn.com.jcs.leetCode.problem13.romanToInteger;
...
public class Solution {
	public int romanToInt(String s) {
		...
		for (int i = 0; i < romans.length; i++) {
			int length = s.length() == 1 ? 1 : romans[i].length();
			if (s.startsWith(romans[i]) && ((i != romans.length) && !s.startsWith(romans[i] + 1))) {
				for (int j = 0; j < 3; j++) {
					if ("".equals(s) || !s.substring(0, length).equals(romans[i])) {
						break;
					}
					answer += map.get(s.substring(0, length));
