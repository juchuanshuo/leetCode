package cn.com.jcs.leetCode.problem682.BaseballGame;
 ...
 * 13. Round 7: You could get 9 + 5 = 14 points. The sum is 27. Note: The size
 * of the input list will be between 1 and 1000. Every integer represented in
...
public class Solution {
	public int calPoints(String[] ops) {
		...
		for (int i = 0; i < ops.length; i++) {
			switch (ops[i]) {
			case "C":
				result -= Integer.parseInt(valid.get(valid.size()-1));
				valid.remove(valid.get(valid.size() - 1));
				break;
			case "D":
				result += 2*Integer.parseInt(valid.get(valid.size()-1));
				valid.add(2*Integer.parseInt(valid.get(valid.size()-1))+"");
				break;
			case "+":
				result += Integer.parseInt(valid.get(valid.size()-1)) + Integer.parseInt(valid.get(valid.size()-2));
				valid.add((Integer.parseInt(valid.get(valid.size()-1)) + Integer.parseInt(valid.get(valid.size()-2)))+"");
				break;
			default:
