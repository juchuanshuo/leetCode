...
public class Solution {
	...
	public boolean canVisitAllRooms(List<List<Integer>> rooms) {
		...
		Set<Integer> openedRooms = new HashSet<Integer>();
		Set<Integer> ownKeys = new HashSet<Integer>();
		ownKeys.addAll(rooms.get(0));
		ownKeys.add(0);
		openedRooms.add(0);
		while (true) {
			openedRooms.addAll(ownKeys);
			for (int a : openedRooms) {
				ownKeys.addAll(rooms.get(a));
			}
			if (openedRooms.size() == rooms.size()) {
				return true;
			}
			if (openedRooms.size() == ownKeys.size() && openedRooms.size() != rooms.size()) {
				return false;
			}
		}
	}
