package cn.com.jcs.leetCode.problem789.escapeTheGhosts;
...
public class Solution {
	public boolean escapeGhosts(int[][] ghosts, int[] target) {
		/*
		 * for (int[] ghost : ghosts) { if ((ghost[0] - 0) <= 1 || (ghost[1] -
		 * 0) <= 1) { return false; } }
		 */
		int distance = target[0] + target[1];
		int[] ghostDistances = new int[ghosts.length];
		for (int i = 0; i < ghosts.length; i++) {
			ghostDistances[i] = Math.abs(target[0] - ghosts[i][0]) + Math.abs(target[1] - ghosts[i][1]);
		}
		for (int dis : ghostDistances) {
			if (dis <= distance) {
				return false;
			}
		}
